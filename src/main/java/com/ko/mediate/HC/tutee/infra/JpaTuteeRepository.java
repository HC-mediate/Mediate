...
public interface JpaTuteeRepository extends JpaRepository<Tutee, Long> {
  ...
  @Query("SELECT t FROM Tutee t JOIN FETCH t.account a WHERE t.account.email IN :accountEmails")
  List<Tutee> findAllByAccountEmails(@Param("accountEmails") List<String> emails);
}
